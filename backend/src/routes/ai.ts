...
router.get('/chat/history', verifyToken, async (req: AuthRequest, res: Response) => {
  try {
    const history = await ChatMessage.find({ userId: req.userId }).sort({ createdAt: 1 });
...
router.post('/chat', verifyToken, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const previousMessages = await ChatMessage.find({ userId: req.userId })
      .sort({ createdAt: -1 })
      .limit(10);
    ...
    const lastMonthExpensesExact = await Expense.find({
      userId: req.userId,
    ...
    const last10Expenses = await Expense.find({ userId: req.userId })
...
function statsEndDate(date: Date) {
